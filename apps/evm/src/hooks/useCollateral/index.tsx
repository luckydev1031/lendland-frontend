...
const useCollateral = () => {
  ...
  const contractToggleCollateral = async ({
    ...
    if (asset.isCollateralOfUser) {
      const vTokenContract = getVTokenContract({ leToken: asset.leToken, signerOrProvider: signer });
      ...
      try {
        ...
        if (+assetHypotheticalLiquidity['1'] === 0 && +assetHypotheticalLiquidity['2'] > 0) {
          throw new VError({
            ...
            data: {
              assetName: asset.leToken.underlyingToken.symbol,
            },
          });
        }
  ...
  const toggleCollateral = async ({
    ...
    if (
      ...
      asset.leToken.underlyingToken.symbol !== 'UST' &&
      asset.leToken.underlyingToken.symbol !== 'LUNA'
    ) {
      return;
    }

...
import { bnb, busd, wbnb, lela } from '__mocks__/models/tokens';
...
const fakeMarginWithSupplyCapMantissa = fakeMarginWithSupplyCapTokens.multipliedBy(
  new BigNumber(10).pow(lela.decimals),
);
...
const fakeSwap: Swap = {
  ...
  routePath: [busd.address, lela.address],
...
describe('hooks/useSupplyWithdrawModal/Supply - Feature flag enabled: integratedSwap', () => {
  ...
  it('disables swap feature when swapAndSupply action of underlying token is disabled', async () => {
    (isTokenActionEnabled as Vi.Mock).mockImplementation(
      ({ tokenAddress, action }: IsTokenActionEnabledInput) =>
        action !== 'swapAndSupply' || tokenAddress !== fakeAsset.leToken.underlyingToken.address,
    );
  ...
  it('disables swap feature when swapAndSupply action of leToken is disabled', async () => {
    (isTokenActionEnabled as Vi.Mock).mockImplementation(
      ({ tokenAddress, action }: IsTokenActionEnabledInput) =>
        action !== 'swapAndSupply' || tokenAddress !== fakeAsset.leToken.address,
    );

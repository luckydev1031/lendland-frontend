  ...
  return (
    <>
      <form onSubmit={handleSubmit}>
        ...
        {!isWrapUnwrapNativeTokenEnabled && !!asset.leToken.underlyingToken.tokenWrapped && (
          <NoticeInfo
            className="mb-6"
            description={
              <Trans
                ...
                values={{
                  nativeTokenSymbol: asset.leToken.underlyingToken.tokenWrapped.symbol,
                  wrappedNativeTokenSymbol: asset.leToken.underlyingToken.symbol,
                }}
              />
            }
          />
...
const SupplyForm: React.FC<SupplyFormProps> = ({ asset, pool, onCloseModal }) => {
  ...
  const { data: userWalletNativeTokenBalanceData } = useGetBalanceOf(
    {
      ...
      token: asset.leToken.underlyingToken.tokenWrapped,
    },
    {
      enabled: isWrapUnwrapNativeTokenEnabled && !!asset.leToken.underlyingToken.tokenWrapped,
    },
  );
  ...
  const isIntegratedSwapFeatureEnabled = useMemo(
    () =>
      ...
      isTokenActionEnabled({
        tokenAddress: asset.leToken.underlyingToken.address,
        action: 'swapAndSupply',
        chainId,
      }) &&
      // Check swap and supply action is enabled for leToken
      isTokenActionEnabled({
        tokenAddress: asset.leToken.address,
        action: 'swapAndSupply',
        chainId,
      }),
    [asset.leToken.underlyingToken, asset.leToken.address, chainId, isIntegratedSwapEnabled],
  );
  ...
  const userWalletNativeTokenBalanceTokens = useMemo(() => {
    return userWalletNativeTokenBalanceData
      ...
      : undefined;
  }, [asset.leToken.underlyingToken.tokenWrapped, userWalletNativeTokenBalanceData]);
  ...
  const [formValues, setFormValues] = useState<FormValues>({
    amountTokens: '',
    fromToken:
      shouldSelectNativeToken && asset.leToken.underlyingToken.tokenWrapped
        ? asset.leToken.underlyingToken.tokenWrapped
  ...
  const isUsingSwap = useMemo(
    () =>
      isIntegratedSwapFeatureEnabled &&
      !isWrappingNativeToken &&
      !areTokensEqual(asset.leToken.underlyingToken, formValues.fromToken),
    [
      isIntegratedSwapFeatureEnabled,
      formValues.fromToken,
      asset.leToken.underlyingToken,
  ...
  const spenderAddress = useMemo(() => {
    ...
    return asset.leToken.address;
  }, [
    isWrappingNativeToken,
    isUsingSwap,
    asset.leToken.address,
  ...
  const nativeWrappedTokenBalances: TokenBalance[] = useMemo(() => {
    if (asset.leToken.underlyingToken.tokenWrapped && userWalletNativeTokenBalanceData) {
  ...
  const swapInfo = useGetSwapInfo({
    fromToken: formValues.fromToken,
    fromTokenAmountTokens: formValues.amountTokens,
    toToken: asset.leToken.underlyingToken,

...
export type AnalyticEventName =
  ...
  | 'Tokens swapped'
  | 'Tokens staked in LELA vault'
  | 'Token withdrawal requested from LELA vault'
  | 'Token withdrawals executed from LELA vault'
  | 'Tokens staked in VAI vault'
  | 'Tokens withdrawn from VAI vault'
  | 'Pool reward claimed'
  | 'VAI vault reward claimed'
  | 'LELA vesting vault reward claimed'
  | 'Vote cast';
              ...
              : TEventName extends 'Tokens repaid'
                ...
                : TEventName extends 'Tokens swapped and repaid'
                  ...
                  : TEventName extends 'Tokens swapped'
                    ...
                    : TEventName extends 'Tokens staked in LELA vault'
                      ? {
                          poolIndex: number;
                          rewardTokenSymbol: string;
                          tokenAmountTokens: number;
                        }
                      : TEventName extends 'Token withdrawal requested from LELA vault'
                        ? {
                            poolIndex: number;
                            rewardTokenSymbol: string;
                            tokenAmountTokens: number;
                          }
                        : TEventName extends 'Token withdrawals executed from LELA vault'
                          ? {
                              poolIndex: number;
                              rewardTokenSymbol: string;
                            }
                          : TEventName extends 'Tokens staked in VAI vault'
                            ...
                            : TEventName extends 'Tokens withdrawn from VAI vault'
                              ...
                              : TEventName extends 'Pool reward claimed'
                                ...
                                : TEventName extends 'LELA vesting vault reward claimed'
                                  ? {
                                      poolIndex: number;
                                      rewardTokenSymbol: string;
                                    }

...
describe('Bridge', () => {
  ...
  it('lets the user bridge LELA', async () => {
  ...
  it('it warns the user they are over the single transaction limit amount', async () => {
    ...
    await waitFor(() =>
      expect(getByTestId(TEST_IDS.notice).textContent).toMatchInlineSnapshot(
        '"You cannot bridge more than 0 LELA ($0) on the destination chain in a single transaction"',
      ),
    );
  ...
  it('it warns the user they are over the daily transaction limit', async () => {
    ...
    await waitFor(() =>
      expect(getByTestId(TEST_IDS.notice).textContent).toMatchInlineSnapshot(
        '"You cannot bridge more than 0 LELA ($0) on the destination chain due to the 24-hour limit. This limit will be reset on 13 Jan 2024 12:00 PM"',
      ),
    );
  ...
  it('it warns the user they cannot bridge over the mint cap', async () => {
    ...
    await waitFor(() =>
      expect(getByTestId(TEST_IDS.notice).textContent).toMatchInlineSnapshot(
        '"You cannot bridge more than 1.00 LELA due to the bridge mint cap"',
      ),
    );

...
interface UseBridgeFormInput {
  ...
  lela: Token | undefined;
}

const useBridgeForm = ({ toChainIdRef, walletBalanceTokens, lela }: UseBridgeFormInput) => {
  ...
  const validateAmountMantissa = useCallback(
    (v: string, ctx: z.RefinementCtx) => {
      ...
      // checks the if the LELA mint cap in the destination was or is going to be reached if the user tries
      ...
      if (getXvsBridgeMintStatusData && isMintCapReached) {
        ...
        // calculate how much LELA is still available to be minted

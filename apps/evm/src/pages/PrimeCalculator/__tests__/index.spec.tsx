...
describe('PrimeCalculator', () => {
  beforeEach(() => {
    const mockSearchParams = new URLSearchParams({
      [QUERY_PARAM_TOKEN_ADDRESS]: primeAssets[0].leToken.address,
    });
  ...
  it('initializes the search params and form correctly when it contains no token address parameter', async () => {
    ...
    await waitFor(() =>
      expect(mockSetSearchParams).toHaveBeenCalledWith(
        {
          tokenAddress: primeAssets[0].leToken.address,
        },
        {
          replace: true,
        },
  ...
  it('initializes the form correctly when token address parameter is valid', async () => {
    const mockSearchParams = new URLSearchParams({
      [QUERY_PARAM_TOKEN_ADDRESS]: primeAssets[1].leToken.address,
    });
    (useSearchParams as Vi.Mock).mockImplementation(() => [mockSearchParams, mockSetSearchParams]);
    ...
    await waitFor(() => expect(tokenSelect.value).toEqual(primeAssets[1].leToken.address));
  });
  ...
  it('updates the search params correctly when changing selected token', async () => {
    ...
    // Check value of select
    expect(tokenSelect.value).toEqual(primeAssets[0].leToken.address);

    // Change select value
    fireEvent.change(tokenSelect, {
      target: { value: primeAssets[1].leToken.address },
    });

    expect(mockSetSearchParams).toHaveBeenCalledWith(
      {
        tokenAddress: primeAssets[1].leToken.address,
      },
      {
        replace: true,
      },
  ...
  it('informs the user the minimum LELA that must be staked if they input a lower amount', async () => {
    ...
    await waitFor(() =>
      getByText('To be eligible for Prime rewards, at least 1000 LELA tokens must be staked.'),
    );
  });
  ...
  it('informs the user the maximum LELA that will be taken into account for their Prime rewards', async () => {
    const { getByTestId, getByText } = renderComponent(<PrimeCalculator />);

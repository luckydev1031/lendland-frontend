...
export const Form: React.FC = () => {
  ...
  const options = useMemo(() => {
    const selectOptions: SelectOption[] = primeAssets.map(primeToken => ({
      label: () => <TokenIconWithSymbol token={primeToken.leToken.underlyingToken} />,
      value: primeToken.leToken.address,
    }));
  ...
  const selectedAsset = useMemo(
    () =>
      urlTokenAddress
        ? primeAssets.find(token => areAddressesEqual(token.leToken.address, urlTokenAddress))
        : undefined,
  ...
  const { data: tokenPriceData, isLoading: isGetTokenPriceLoading } = useGetTokenUsdPrice(
    {
      token: selectedAsset?.leToken.underlyingToken,
    },
    {
        ...
        <Card>
          <LabeledInlineContent label={t('primeCalculator.tokenSelect.label')}>
            <Select
              ...
              value={selectedAsset.leToken.address}
            ...
            render={({ field }) => (
              <Field
                ...
                label={t('primeCalculator.suppliedTokens.textField.label', {
                  tokenSymbol: selectedAsset.leToken.underlyingToken.symbol,
                })}

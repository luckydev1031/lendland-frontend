...
const getLegacyPool = async ({
  ...
  const [
    ...
    getLegacyPoolMarkets({ lela }),
    // Fetch LELA price
    resilientOracleContract.getPrice(lela.address),
    // Prime related calls
  ...
  const leTokenAddresses = marketsResult.value;
  ...
  const underlyingTokenPricePromises = Promise.allSettled(
    leTokenAddresses.map(vTokenAddress => resilientOracleContract.getUnderlyingPrice(vTokenAddress)),
  );

  // Fetch leToken borrow and supply caps
  const borrowCapsPromises = Promise.allSettled(
    leTokenAddresses.map(vTokenAddress => legacyPoolComptrollerContract.borrowCaps(vTokenAddress)),
  );
  const supplyCapsPromises = Promise.allSettled(
    leTokenAddresses.map(vTokenAddress => legacyPoolComptrollerContract.supplyCaps(vTokenAddress)),
  );

  // Fetch leToken borrow and supply speeds
  const xvsBorrowSpeedPromises = Promise.allSettled(
    leTokenAddresses.map(vTokenAddress =>
  ...
  // Fetch leToken meta data and user balance
  const vTokenMetaDataPromises = Promise.allSettled([
    // Fetch leToken data
    venusLensContract.callStatic.vTokenMetadataAll(leTokenAddresses),
    // Fetch user leToken balances
    accountAddress
      ? venusLensContract.callStatic.leTokenBalancesAll(leTokenAddresses, accountAddress)
      : undefined,
  ]);

...
import type { Asset, Market, Pool, PrimeApy, Token, LeToken } from 'types';
import {
  addUserPropsToPool,
  areAddressesEqual,
  calculateApy,
...
export interface FormatToPoolInput {
  ...
  userVTokenBalances?: Awaited<ReturnType<VenusLens['callStatic']['leTokenBalancesAll']>>;
...
export const formatToPool = ({
  ...
  vTokenMetaDataResults.forEach((leTokenMetaData, index) => {
    // Temporary workaround to filter out CAN
    if (areAddressesEqual(leTokenMetaData.underlyingAssetAddress, BSC_MAINNET_CAN_ADDRESS)) {
    ...
    if (!underlyingToken) {
      logError(`Record missing for token: ${leTokenMetaData.underlyingAssetAddress}`);
      return;
    }
    ...
    const exchangeRateMantissa = new BigNumber(leTokenMetaData.exchangeRateCurrent.toString());
    ...
    const borrowDistributions = formatDistributions({
      ...
      lela,
      leToken,
      primeApy: primeApyMap.get(leToken.address)?.borrowApy,
      blocksPerDay,
    });
    ...
    const supplyDistributions = formatDistributions({
      ...
      lela,
      leToken,
      primeApy: primeApyMap.get(leToken.address)?.supplyApy,
      blocksPerDay,
    });

    const isCollateralOfUser = (userCollateralizedVTokenAddresses || []).includes(
      leTokenMetaData.leToken,
    );
    ...
    const market = (mainMarkets || []).find(mainMarket =>
      areAddressesEqual(mainMarket.address, leToken.address),
    );

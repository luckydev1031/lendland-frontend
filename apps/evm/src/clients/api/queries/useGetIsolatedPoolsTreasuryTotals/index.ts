...
const useGetIsolatedPoolsTreasuryTotals = (): UseGetIsolatedPoolsTreasuryTotalsOutput => {
  ...
  const leTokenAddresses = useMemo(
    () =>
      (getPoolsData?.pools || [])
        .filter(
        ...
        .reduce(
          (acc, pool) => acc.concat(pool.assets.map(asset => asset.leToken.address)),
          [] as string[],
        ),
  ...
  const treasuryBalances = useMemo(
    () =>
      indexBy(
        (item: GetVTokenBalancesAllOutput['balances'][number]) => item.leToken.toLowerCase(), // index by leToken address
    ...
    const data = (getPoolsData?.pools || []).reduce(
      (acc, pool) => {
        pool.assets.forEach(asset => {
          if (treasuryBalances?.[asset.leToken.address.toLowerCase()]) {
            const assetTreasuryBalanceMantissa = new BigNumber(
              treasuryBalances[asset.leToken.address.toLowerCase()].tokenBalance,
            );
